        ...
        Row {
            Column {
                (if (showRealTemp) temperature?.temperature else temperature?.feelsLikeTemperature)?.let {
                    Text(
                        text = temperatureUnit.getValueText(context, it),
...
private fun TemperatureChart(
    ...
    if (mappedValues.size >= ChartDisplay.CHART_MIN_COUNT) {
        ...
        val minY = remember(mappedValues, showRealTemp, normals) {
            ...
            }.minus(5.0).roundToInt().toDouble() // TODO: Make this a const
        }
        ...
        val endAxisValueFormatter = CartesianValueFormatter { _, value, _ ->
            SettingsManager.getInstance(context).temperatureUnit.getShortValueText(context, value)
        }
            ...
            modelProducer.runTransaction {
                lineSeries {
                    ...
                    if (!showRealTemp) {
                        series(
                            x = mappedValues.keys,
                            y = mappedValues.values.map { it.temperature!!.temperature!! }
                        )
                    }
                }
            }
        ...
        BreezyLineChart(
            ...
            persistentListOf(
                persistentMapOf(
                    47f to Color(71, 14, 0),
                    30f to Color(232, 83, 25),
                    21f to Color(243, 183, 4),
                    10f to Color(128, 147, 24),
                    1f to Color(68, 125, 99),
                    0f to Color(93, 133, 198),
                    -4f to Color(100, 166, 189),
                    -8f to Color(106, 191, 181),
                    -15f to Color(157, 219, 217),
                    -25f to Color(143, 89, 169),
                    -40f to Color(162, 70, 145),
                    -55f to Color(202, 172, 195),
                    -70f to Color(115, 70, 105)
                ),
                persistentMapOf(
            ...
            trendHorizontalLines = buildMap {
                normals?.let {
                    it.daytimeTemperature?.let { normal ->
                        put(normal, context.getString(R.string.temperature_normal_short))
                    }
